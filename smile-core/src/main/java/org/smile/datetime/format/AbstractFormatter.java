...
public abstract class AbstractFormatter implements JdtFormatter {
	 ...
	 */
	protected abstract String convertPattern(int patternIndex, DateTime jdt);

	/**
	 * {@inheritDoc}
	 * @see JdtFormatter#convert(DateTime, String)
	 */
	public String convert(DateTime jdt, String format) {
	...
	public DateTimeStamp parse(String value, String format) {
		...
		if (valueLen == formatLen) {
			...
			for (char valueChar : valueChars) {
				if (CharUtils.isDigit(valueChar) == false) {
					useSeparators = true;
					break;
				}
			}
