...
import org.smile.io.StringReader;
...
public class TagTokenizer {
    ...
    public static interface TokenHandler {
         ...
         */
        void tag(TagInfo tag) throws IOException;
    ...
    public static enum Token {
        ...
        LT_OPEN_MAGIC_COMMENT, LT_CLOSE_MAGIC_COMMENT, EOF,COMMENT
    }
    ...
    public TagTokenizer(CharSequence input, TokenHandler handler) {
        this.handler = handler;
        ...
        try {
            while (true) {
                ...
                if (token == Token.EOF) {
                    ...
                    // Token "<" - start of tag
                    parseTag(TagInfo.Type.OPEN);
                } else if (token == Token.LT_OPEN_MAGIC_COMMENT) {
                    // Token "<!--[" - start of open magic comment
                    parseTag(TagInfo.Type.OPEN_CONDITIONAL_COMMENT);
                } else if (token == Token.LT_CLOSE_MAGIC_COMMENT) {
                    // Token "<![" - start of close magic comment
                    parseTag(TagInfo.Type.CLOSE_CONDITIONAL_COMMENT);
                } else if (token ==Token.COMMENT) {//注释的时候不解析，需把前面有文字解析完成
                	flushText();
                }else {
    ...
    private void parseTag(TagInfo.Type type) throws IOException {
        // Start parsing a TAG
    ...
    private void parseFullTag(TagInfo.Type type, String name, int start) throws IOException {
        Token token;
        while (true) {
            skipWhiteSpace();
            if (pushbackToken == Token.UNKNOWN) {
    ...
    private void parsedTag(TagInfo.Type type, String name, int start, int length) throws IOException {
        flushText();
        this.type = type;
        ...
        reusableToken.attributes.clear();
    }
    ...
    private void parsedAttribute(String name, String value, boolean quoted) {
        if (quoted) {
        	reusableToken.attributes.put(name,value.substring(1, value.length() - 1));
        } else {
        	reusableToken.attributes.put(name, value);
        }
    }
     ...
     *
     */
    private class ReusableToken extends CustomTagInfo {
