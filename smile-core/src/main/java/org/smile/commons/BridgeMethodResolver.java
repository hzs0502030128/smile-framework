...
public abstract class BridgeMethodResolver {
	...
	public static Method findBridgedMethod(Method bridgeMethod) {
		...
		Set<Method> methods = MethodUtils.getAllDeclaredMethods(bridgeMethod.getDeclaringClass());
	...
	private static boolean isResolvedTypeMatch(Method genericMethod, Method candidateMethod, Class<?> declaringClass) {
		...
		for (int i = 0; i < candidateParameters.length; i++) {
			ParameterizedType pt=(ParameterizedType)genericParameters[i];
			Class genericClazz=(Class)pt.getRawType();
			Class<?> candidateParameter = candidateParameters[i];
			if (candidateParameter.isArray()) {
				// An array type: compare the component type.
				if (!candidateParameter.getComponentType().equals(genericClazz.getComponentType())) {
					return false;
				}
			}
			// A non-array type: compare the type itself.
			if (!candidateParameter.equals(genericClazz)) {
				return false;
			}
		}
		return true;
	...
	private static Method findGenericDeclaration(Method bridgeMethod) {
		...
		Class<?>[] interfaces = ClassTypeUtils.getAllInterfaces(bridgeMethod.getDeclaringClass());
