 ...
 */
public class ByteBand implements Output{
	 ...
	 */
	public ByteBand() {
		this.minChunkLen = IOUtils.DEFAULT_BUFF_SIZE;
	}
	 ...
	 */
	public ByteBand append(byte[] array, int off, int len) {
		int end = off + len;
		if ((off < 0)
				|| (len < 0)
				|| (end > array.length)) {
	 ...
	 * Appends <code>byte</code> array to buffer.
	 */
	public ByteBand append(byte[] array) {
	 ...
	 */
	public ByteBand append(byte element) {
	 ...
	 */
	public ByteBand append(ByteBand buff) {
		if (buff.size == 0) {
			return this;
		}
