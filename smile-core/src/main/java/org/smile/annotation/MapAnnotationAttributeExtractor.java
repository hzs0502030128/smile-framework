...
public class MapAnnotationAttributeExtractor extends AbstractAliasAwareAnnotationAttributeExtractor<Map<String, Object>> {
	...
	MapAnnotationAttributeExtractor(Map<String, Object> attributes, Class<? extends Annotation> annotationType,
			 AnnotatedElement annotatedElement,OverWriteAnnotation overwriter) {

		super(annotationType, annotatedElement, enrichAndValidateAttributes(attributes, annotationType),overwriter);
	}
	...
	private static Map<String, Object> enrichAndValidateAttributes(
		...
		for (Method attributeMethod : AnnotationUtils.getAttributeMethods(annotationType)) {
			...
			if (!ClassTypeUtils.isAssignable(requiredReturnType, actualReturnType)) {
