 ...
 * {@linkplain #getAttributeValue extracting} annotation attribute values from
 * an underlying {@linkplain #getSource source} such as an {@code Annotation} or
 * a {@code Map}.
 *
 * @author Sam Brannen
 * @since 4.2
 * @param <S>
 *            the type of source supported by this extractor
 * @see SynthesizedAnnotationInvocationHandler
 */
public interface AnnotationAttributeExtractor<S> {

	/**
	 * Get the type of annotation that this extractor extracts attribute values
	 * for.
	 */
	...
	/**
	 * Get the attribute value from the underlying {@linkplain #getSource
	 * source} that corresponds to the supplied attribute method.
	 * 
	 * @param attributeMethod
	 *            an attribute method from the annotation type supported by this
	 *            extractor
