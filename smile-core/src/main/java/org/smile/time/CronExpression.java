 ...
 */
public final class CronExpression implements FireOnExpression,Serializable, Cloneable {
	...
	public boolean isValidTime(long times) {
		...
		Date fta =getTimeAfter(new Date(times - 1000L));

		while (fta.before(testTime)) {
			fta = getTimeAfter(fta);
		}
		c.setTime(fta);
		c.set(Calendar.MILLISECOND, 0);
	...
	public Date getNextValidTimeAfter(long times) {
		return getTimeAfter(new Date(times));
	}
